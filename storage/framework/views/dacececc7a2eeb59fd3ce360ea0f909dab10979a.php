    ...
    <div class="row">
      <div class="col-md-12">
        <ul class="nav nav-tabs" role="tablist">
          <?php
                        $news = DB::table('news_cat')->where('type_news_cat',1)->orderby('order','asc')->orderby('last_date','DESC')->get();
                        $num = $news[0]->id;
                        if(isset($_GET['tid'])){
                            $num = $_GET['tid'];
                        }
      ...
      <div class="col-md-12">
        <div class="tab-content">
          <?php
                        ...
                        $news1 = DB::table('news')->where('cat', $num)->orderBy('last_date_new', 'DESC')->orderBy('orders', 'asc')->paginate(10);
          ?>
              ...
              <div class="row">
                <?php
                            foreach ($news1 as $newsOut) {
                              ...
                              if($newsOut->img !="" && $num != 47){ $imgh= url("public/uploadimg").'/'.$newsOut->img;}
                            ...
                            ?> </div>
              <div class="pagination center-block"><?php echo str_replace('/?', '?',$news1->render()); ?></div>
            </div>
            <?php
                        $blog->tabsPaneOff();
                        ?>
...
function tabsPanels($h1, $tabid,$num) {
 ...
 if($tabid == $num){$active = "active";}
    echo '<li class="" ><a class="nav-link mbr-fonts-style display-7 '.$active.'" style=""  href="'.url('news').'?tid='.$tabid.'" >
    ' . $h1 . '</a></li>';
}
?>