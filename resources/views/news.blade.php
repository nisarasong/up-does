...
$newsCat_type = 1;
$tabMenu = DB::table('news_cat')->where('type_news_cat', 1)->orderby('order', 'asc')->orderby('last_date', 'DESC')->get();
...
<body>
  <br><br>
          ...
          $custom->createTabMenu($tabMenu,$tabActiveId,$locale,0,'news');
          ...
          ?><div class="pagination center-block">{!! str_replace('/?', '?',$dataQuery->render()) !!}</div><?php
          echo '</div>';
          $blog->tabsPaneOff();
          ?>
